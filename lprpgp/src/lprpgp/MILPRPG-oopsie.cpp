	...
	for (int i = 0; i < varCount; ++i) {
		if (RPGBuilder::getDominanceConstraints()[i] != E_METRIC && RPGBuilder::getDominanceConstraints()[i] != E_IRRELEVANT) {
			...
			if (toFill[i] < maxNeeded[i]) {
				...
				maximise[0] = 1.0;
				optVar[0] = get_lp_index(lp, minMaxVars[i].Vvar + Nrows);
...
//				memcpy(startingPoint, minMaxVars[i].maximisationGuess, (minMaxVars[i].columnsInGuess) * sizeof(REAL));
	...
	//				delete [] startingPoint;
					...
					const double mv = get_var_primalresult(lp, Nrows + minMaxVars[i].Vvar);
					toFill[i] = mv;
					maxRealVec[i] = true;					
				}
...
//				for (int t = 0 ; t <= colCount; ++t) startingPoint[t] = 0.0;
	...
	//				delete [] startingPoint;
					...
					const double mvTwo = get_var_primalresult(lp, Nrows + minMaxVars[i].Vvar);
					toFill[i + varCount] = -mvTwo;
					minRealVec[i] = true;					
...
void MILPRPG::getActionsFor(const double & TS, const int & noPPi, const bool & min, const double & RHS, list<pair<int, double> > & toReturn) {
	...
	const int Nrows = get_Norig_rows(lp);
	int PPi = get_lp_index(lp, noPPi + 1 + Nrows);

	if (!PPi) {
		return; // no actions needed to achieve any value of a presolvable variable
	}
...
*/
	REAL oldlowbo = get_lowbo(lp, PPi);
	REAL oldupbo = get_upbo(lp, PPi);

	double boundVal = (RHS == -1.0 ? (min ? toUse->min[noPPi - 1] : toUse->max[noPPi - 1]) : RHS);
	...
	if (!toUse->nowExtracting) {
		...
		const int origCols = get_Norig_columns(lp);

		for (int v = varCount + 1; v <= origCols; ++v) {
		...
		for (int v = 0; v < varCount; ++v) {
			if (toUse->minReal[v]) {
				const int RI = get_lp_index(lp, v + 1 + Nrows);
				if (RI) {
					set_lowbo(lp, RI, toUse->min[v]);
				}
			}
			if (toUse->maxReal[v]) {
				const int RI = get_lp_index(lp, v + 1 + Nrows);
				if (RI) {
					set_upbo(lp, RI, toUse->max[v]);
				}
			}
		}
//
...
//		}
	...
	set_bounds(lp, PPi, boundVal, boundVal);
	solve(lp);


	const int lpColCount = get_Ncolumns(lp);
	const int lpOrigColCount = get_Norig_columns(lp);
	...
	for (int v = varCount; v < lpOrigColCount; ++v) {
		const int RI = get_lp_index(lp, v + 1 + Nrows);
		REAL var = (RI ? results[RI] : get_var_primalresult(lp, Nrows + v + 1));
		if (var > 0.001) {
			toReturn.push_back(pair<int,double>(v, var));
		}
	...
	PPi = get_lp_index(lp, noPPi + 1 + Nrows);
	if (PPi) {
		set_bounds(lp, PPi, oldlowbo, oldupbo);
	}
};
