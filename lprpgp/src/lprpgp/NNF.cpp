...
void NNFVisitor::visit_AND(NNFNode_AND * andNode) {
...
void NNFVisitor::visit_OR(NNFNode_OR * orNode) {
...
void NNFVisitor::visit_NOT_Literal(NNFNode_NOT_Literal *) {};
void NNFVisitor::visit_Literal(NNFNode_Literal *) {};
void NNFVisitor::visit_NOT_Numeric(NNFNode_NOT_Numeric *) {};
void NNFVisitor::visit_Numeric(NNFNode_Numeric *) {};
void NNFVisitor::visit_NOT_RPGNumeric(NNFNode_NOT_RPGNumeric *) {};
void NNFVisitor::visit_RPGNumeric(NNFNode_RPGNumeric *) {};
...
NNFNode * NNFNode_RPGNumeric::negate() {
    NNFNode * const toReturn = new NNFNode_NOT_RPGNumeric(pre,ts);
    pre = 0;
    return toReturn;
};

NNFNode * NNFNode_NOT_RPGNumeric::negate() {
    NNFNode * const toReturn = new NNFNode_RPGNumeric(pre,ts);
    pre = 0;
    return toReturn;
};
...
public:
    ...
    virtual void visit_simple_goal(simple_goal * p)  {
        ...
        if (addLit) {
            ...
            const pair<bool,bool> & staticInfo = LiteralAnalysis::isStatic(addLit);
...
public:
    ...
    virtual void visit_NOT_RPGNumeric(NNFNode_NOT_RPGNumeric * n) {
        if (!staticNumerics) {
            justCreated.push_back(pair<bool,bool>(false,true));
        } else {
...
public:
    ...
    {
        if (simplifyDebug) {
            cout << string(simplifyIndent, ' ') << "¬" << RPGBuilder::getNumericPrecs()[n->getPre()] << endl;
        }        
...
public:
    ...
    virtual void visit_NOT_Literal(NNFNode_NOT_Literal * litNode) {
        ...
        const pair<bool,bool> & currStatic = LiteralAnalysis::isStatic(currLit);
...
public:
    ...
    virtual void processNumeric(list<RPGBuilder::NumericPrecondition> & justOne, const bool & weWant, const VAL::time_spec & ts) {
        int unsat = 0;
        list<int> dest;

        const bool evaluateOkay = RPGBuilder::processPreconditions(commonData, justOne, dest, unsat, -3, VAL::E_AT);
    
        if (!evaluateOkay) {
            justCreated.push_back(pair<NNFNode*,bool>(0,!weWant));
        } else if (dest.empty()) {
            justCreated.push_back(pair<NNFNode*,bool>(0,weWant));
        } else if (dest.size() == 1) {
            if (weWant) {
                NNFNode * newNode = new NNFNode_RPGNumeric(dest.front(),ts);
                justCreated.push_back(pair<NNFNode*,bool>(newNode, true));
            } else {
                NNFNode * newNode = new NNFNode_NOT_RPGNumeric(dest.front(),ts);
                justCreated.push_back(pair<NNFNode*,bool>(newNode, true));
            }
            ...
            if (weWant) {
                conj->addChild(new NNFNode_RPGNumeric(dest.front(),ts));
                conj->addChild(new NNFNode_RPGNumeric(dest.back(),ts));
            } else {
                conj->addChild(new NNFNode_NOT_RPGNumeric(dest.front(),ts));
                conj->addChild(new NNFNode_NOT_RPGNumeric(dest.back(),ts));
            }

            justCreated.push_back(pair<NNFNode*,bool>(conj,true));
        } else {
            assert(false); // really really should never happen
        }
    }

    virtual void visit_Numeric(NNFNode_Numeric * r) {

        list<RPGBuilder::NumericPrecondition> justOne;
        justOne.push_back(*(r->getPre()));

        processNumeric(justOne,true,r->getTS());
    ...
    virtual void visit_NOT_Numeric(NNFNode_NOT_Numeric * r) {
        ...
        processNumeric(justOne,false,r->getTS());
    }

    virtual void visit_NOT_RPGNumeric(NNFNode_NOT_RPGNumeric *r) {
...
pair<NNFNode*,bool> NNFUtils::substituteRPGNumerics(NNFNode* const root,RPGBuilder::BuildingNumericPreconditionData & commonData) {
    NNFRPGNumericSubstituter c(commonData);
    return c.substitute(root);
};
...
public:
    ...
    virtual void visit_NOT_RPGNumeric(NNFNode_NOT_RPGNumeric *r) {
        cells.push_back(make_pair(NNF_Flat::Cell(r->getPre(),false), currParent));
        childDefaultSatisfied.push_back(true);
        ++leafCount;
    }
    virtual void visit_RPGNumeric(NNFNode_RPGNumeric *r) {
...
{
    o << string(nnfWriteIndent, ' ') << RPGBuilder::getNumericPrecs()[pre] << endl;
    return o;
    
}

ostream& NNFNode_NOT_RPGNumeric::write(ostream& o)
{
    o << string(nnfWriteIndent, ' ') << "¬" << RPGBuilder::getNumericPrecs()[pre] << endl;
    return o;
}
