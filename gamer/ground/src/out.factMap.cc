...
void FactMap::flush() {
  ...
  dstream << "  (:requirements :typing" 
////	  << (dur ? " :durative-actions" : "")
	 << ") " << endl;
  ...
  astream << "  (:partition" << endl;

  for(int i = 0; i < groups.size();i++) {
      astream << "   (:fact-group-" << ::makeString(i) << " " 
	      << groups[i]->getString() ;
      astream << "   )" << endl;
  }
...
////      }
////      else
////	dstream << "  (:durative-action " << operators[i]->getString();
      dstream << "  )" << endl;
    }
  }
  dstream << ")" << endl; 
...
//		  cout << " prune" << prefer << endl;
	  ...
	  for(map<string,int>::iterator pos = 
	      ...
	      int value = (isMinimize() ? pos->second : - pos->second);
...
//		  cout << " prune" << prefer << "in domain metric" << endl;
      ...
      if (domain.metric->toString() != "total-time") 
	  pstream << ")" << endl;
  }
  pstream << ")" << endl;
}
